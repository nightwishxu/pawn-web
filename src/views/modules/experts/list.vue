<template>
  <div class="mod-user">
    ...
    <el-table :data="dataList" border stripe v-loading="dataListLoading" @selection-change="selectionChangeHandle"
      ...
      <el-table-column prop="code" header-align="center" align="center" label="专家编号" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="name" header-align="center" align="center" label="专家姓名"></el-table-column>
      <el-table-column prop="phone" header-align="center" align="center" label="专家手机号"></el-table-column>
      <el-table-column prop="types" header-align="center" align="center" label="鉴定类型" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="scope.row.types!='' && scope.row.types!=null">
            <el-tag :key="tag" v-for="tag in scope.row.types.split(',')" type="success" style="margin-right: 2px;">{{tag}}专家</el-tag>
          </span>
        </template>
      </el-table-column>
      <el-table-column fixed="right" header-align="center" align="center" width="150" :label="$t('m.operation')">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{$t('m.edit')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList() {
        ...
        this.$http({
          url: this.$http.adornUrl('/experts/list'),
          method: 'get',
